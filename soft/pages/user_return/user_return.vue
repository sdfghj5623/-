<template>
  <view class="add">
    <form @submit="onSubmit">
      <!-- 表单项 -->
      <view class="item">
        <input v-model="formValue.title" type="text" name="title" placeholder="是否正确" />
      </view>

      <view class="item">
        <input v-model="formValue.leibie" type="text" name="leibie" placeholder="作物状态" />
      </view>

      <view class="item">
        <textarea maxlength="-1" v-model="formValue.content" name="content" placeholder="您的评价"></textarea>
      </view>
...
export default {
  ...
  methods: {
    ...
    onSubmit(event) {
      ...
      formData.append('TF', this.formValue.title);
      formData.append('zhuangtai', this.formValue.leibie || '');
      formData.append('pingjia', this.formValue.content);
      ...
      // 发送请求
      axios.post('http://127.0.0.1:5000/api/insert_shibie_return', formData, {
        headers: {
    ...
    inDisabled(formValue) {
      return !formValue.title || !formValue.leibie || !formValue.content;
    },
